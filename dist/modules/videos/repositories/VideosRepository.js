...
class VideoRepository {
    create(request, response) {
        ...
        mysql_1.pool.getConnection((err, connection) => {
            connection.query("INSERT INTO videos (video_id, user_id, title, description, thumbnail, publishedAt) VALUES (?,?,?,?,?,?)", [(0, uuid_1.v4)(), user_id, title, description, thumbnail, publishedAt], (error, result, fields) => {
                connection.release();
                if (error) {
    ...
    searchVideos(request, response) {
        ...
        mysql_1.pool.getConnection((err, connection) => {
            connection.query("SELECT * FROM videos WHERE title LIKE ? OR description LIKE ?", [`%${search}%`, `%${search}%`], (error, results, fields) => {
                connection.release();
                if (error) {
                    return response
                        .status(400)
